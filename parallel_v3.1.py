...
def configureFFmpeg():
    ...
    if codecs == 'A':
        ...
        print("\nThis metrics is based on AMD Ryzen 5 6600H\n(results may vary)")
        speed = input("\nInput a character either: ultrafast, veryfast, fast, medium, slow OR , veryslow: ")
        
    elif codecs == 'C':

        codecs = 'libx264'
        print(codecs + " selected. Input speed to use:")
        print("TIP: Speed 'veryslow' = ~10 fps")
        print("\nThis metrics is based on AMD Ryzen 5 6600H\n(results may vary)")
        speed = input("\nInput a character either: ultrafast, veryfast, fast, medium, slow OR , veryslow: ")
...
subprocess.run('ffmpeg -nostats -nostdin -hide_banner -loglevel quiet -i '+ videoPath +' -vn -c:a aac -map 0:1 -map 0:2? -map 0:3? -b:a 256k -af dynaudnorm=f=10:g=3:m=20 -movflags use_metadata_tags \"' + m4aPath + "\"", shell=True)
...
subprocess.run('ffmpeg -hide_banner -f concat -safe 0 -i concatList.txt -i \"'+ m4aPath +'\" -c copy -map 0:0 -map 1:0 -map 1:1? -map 1:2? -movflags use_metadata_tags ' + videoFNAr[0] + dirSlash + videoFNb4split[0] + '_AV1.mp4\"', shell=True)
